...
@testset "continuity_constraint.jl" begin
    ...
    # partials
    @test partials(cc, X1) == AstroTOAST.__dCC_dx1
    @test partials(cc, X2) == AstroTOAST.__dCC_dx2
    @test partials(cc, X3) == AstroTOAST.__no_partial
    @test partials(cc, T) == AstroTOAST.__dCC_dt
    @test partials(ccf, Tf) == AstroTOAST.__no_partial

    # variables in an XVector still result in the right partial function being called
    @test partials(cc, xv[1]) == AstroTOAST.__dCC_dx1 
    @test partials(cc, xv[2]) == AstroTOAST.__dCC_dx2
    @test partials(cc, xv[3]) == AstroTOAST.__dCC_dt

    # partial derivative functions
    @test AstroTOAST.__dCC_dx1(cc) ≈ 
    ...
    @test AstroTOAST.__dCC_dx2(cc) == -I(6)
    @test AstroTOAST.__no_partial(cc) == zeros(6,6)

    constvec = [0.47917602104423834, -0.1623384291766108, 0.0,
                1.26211013132958, -3.6791419243357506, -0.0]
    @test AstroTOAST.__dCC_dt(cc) ≈ constvec atol=1e-12
